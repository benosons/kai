$( document ).ready(function() {
  ...
  const page = $('#page').val();
  $('#menu-'+page+'-ssdi-input').addClass('mm-active');
  loadindikator(page);
          ...
          if(param == 'ess'){
            $('#list-'+param+'-ssdi').DataTable({
                    ...
                    aoColumns: [
                        ...
                        { 'mDataProp': 'id'},
                        { 'mDataProp': 'nama_module'},
                        { 'mDataProp': 'judul_module'},
                        { 'mDataProp': 'deskripsi'},
                        { 'mDataProp': 'aktif'},
                        { 'mDataProp': 'type'},
                          ...
                          mRender: function ( data, type, row ) {
                            var el =
                              `<div role="group" class="btn-group-sm btn-group btn-group-toggle">
                                                        <button data-toggle="tooltip" title="Edit !" type="button" class="btn btn-success" onclick="actionindikator('edit', '`+page+`', '`+row.id+`', '`+row.indikator_type+`', '`+row.indikator_1+`', '`+row.indikator_2+`', '`+row.keterangan+`', '`+row.satuan+`', '`+row.formula+`', '`+row.prioritas+`', '`+row.tahun+`')"><i class="fa fa-edit"></i></button>
                                                        <button data-toggle="tooltip" title="Delete !" type="button" class="btn btn-danger" onclick="actionindikator('delete', '`+page+`', '`+row.id+`')"><i class="fa fa-times"></i></button>
                                                    </div>`;
